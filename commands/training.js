...
// Takes "3:00 PM" or "15:00" and treats it as CST (UTC-6), returns Unix timestamp
function parseTimeCST(timeStr) {
  ...
  // CST = UTC-6, so to convert CST time to UTC we add 6 hours
...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption(o =>
      o.setName('time')
        .setDescription('Start time in CST — e.g. "3:00 PM" or "15:00"')
        .setRequired(true)
    )
    .addStringOption(o =>
      o.setName('notes')
  ...
  async execute(interaction) {
    ...
    // Parse CST time into Unix timestamp
    const timestamp = parseTimeCST(timeStr);
    if (!timestamp) {
      return interaction.reply({
        content: '⚠️ Invalid time format. Please use formats like `3:00 PM` or `15:00`.',
        ephemeral: true,
      });
    }
