...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (!duration)
      return interaction.reply({ content: '⚠️ Invalid duration. Use `10m`, `2h`, or `1d`.', ephemeral: true });
    ...
    collector.on('end', async () => {
      ...
      if (entrantList.length === 0) {
        const noWinnersEmbed = EmbedBuilder.from(giveawayMessage.embeds[0])
      ...
      await channel.send({ embeds: [
        new EmbedBuilder()
          ...
          .setDescription(`Congratulations ${winnerMentions}!\nYou won **${prize}**! Please contact the host to claim your prize. 🎁`)
          .setTimestamp()
      ]});
    });
  },
