...
const { hasAccess } = require('./commands/commandguard');
...
const SKIP_FILES = ['logger.js', 'channelcheck.js', 'globalconfig.js', 'commandguard.js'];
...
try { punishHandler = require('./commands/punish'); } catch (e) {}
...
client.on('interactionCreate', async (interaction) => {
  ...
  // ── Global access guard ───────────────────────────────────────────────────
  // Only members with Sergeant+ or Admin+ roles can use any command
  if (interaction.member && !hasAccess(interaction.member)) {
    return interaction.reply({
      content: '🚫 You do not have permission to use bot commands.',
      ephemeral: true,
    });
  }
  // ─────────────────────────────────────────────────────────────────────────
